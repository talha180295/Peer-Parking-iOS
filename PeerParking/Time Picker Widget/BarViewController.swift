...
//          chartBar.transform = CGAffineTransform(rotationAngle: CGFloat.pi / 2)
        ...
        months = ["12 am", "1 am" , "2 am", "3 am", "4 am", "5 am", "6 am", "7 am", "8 am", "9 am", "10 am", "11 am", "12 pm", "1 pm", "2 pm", "3 pm", "4 pm","5 pm","6 pm","7 pm","8 pm","9 pm", "10 pm","11 pm","12 am"]
...
//              vieww.transform = identity.scaledBy(x: sender.scale, y: sender.scale)
    ...
    override func updateChartData() {
          ...
          self.setDataCount(Int(24))
        }
        
        func setDataCount(_ count: Int) {
            let start = 0
            let end = Int(selectionMax) + 2
            print(selectionMin)
            print(selectionMax)
            let yVal = (start + 1..<count + 1 ).map{ (i) -> BarChartDataEntry in
                print("label = \(i)")
                if (durationLabel.text?.elementsEqual("00h00m"))!{
                    
                    return BarChartDataEntry(x: Double(i), y: 0, icon: UIImage(named: "icon"))

                }
              
               else if i >= end || i <= Int(selectionMin){
                  
                    return BarChartDataEntry(x: Double(i), y: 0, icon: UIImage(named: "icon"))
                    
                    
                }
               return BarChartDataEntry(x: Double(i), y: 50, icon: UIImage(named: "icon"))
